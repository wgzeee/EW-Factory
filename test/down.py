...
def download(var_list, year_list, month=None, day=None, time=None, area=None, dataset="reanalysis-era5-single-levels"):
    '''
    ...
    for var in var_list:
        for year in sorted(year_list, reverse=True):
            req = build_request(var, year, month, day, time, area)
            print(f"正在下载: {var} 年份: {year}")
            client.retrieve(dataset, req).download(f"era5_shandong/{var}/{year}.nc")
            clear_output()

def build_request(var, year, month, day, time, area):
    return {
